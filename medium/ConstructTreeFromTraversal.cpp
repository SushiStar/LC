...
class Solution{
    ...
    private:
        TreeNode* helper(vector<int>& preorder, std::unordered_map<int,int>& mp, int preStart, int inStart, int inEnd){
            ...
            TreeNode* rht = helper(preorder, mp, preStart+bk-inStart+1, bk+1, inEnd);
            root->left = lft;
            root->right = rht;
            return root;
        }
