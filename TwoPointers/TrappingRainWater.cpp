...
public:
    ...
    //  binary search the max value within a range
    int findMax(vector<int> &height, int left, int right) {
        if (left == right) {
            return left;
        } else {
            int mid = (left + right) / 2;
            int res1 = findMax(height, left, mid);
            int res2 = findMax(height, mid + 1, right);
...
public:
    int trap(vector<int> &height) {
        ...
        while (curr < height.size()) {
            if (stk.empty() || height[curr] <= stk.back()) {
                stk.push_back(height[curr]);
                ++curr;
            } else {
                int h = height[stk.back()];
                stk.pop_back();
                if (!stk.empty()) {
                    ret += (std::min(height[stk.back()], height[curr])-h)*(curr-stk.back()-1);
                }
            }
        }
        return ret;
