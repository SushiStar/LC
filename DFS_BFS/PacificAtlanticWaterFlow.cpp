...
public:
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& matrix) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (!atlantic[i][m - 1]) {
                flowAtlantic(i, m - 1, matrix, atlantic);
            }
        }
        
        for (int i = 0; i < m; ++i) {
            if (!pacific[0][i]) {
                flowPacific(0, i, matrix, pacific);
            }
            
            if (!atlantic[n - 1][i]) {
                flowAtlantic(n - 1, i, matrix, atlantic);
            }
        }
...
private:
    ...
    void flowPacific(int i, int j, const vector<vector<int>>& matrix, vector<vector<bool>>& pacific) {
        int n = matrix.size();
        int m = matrix[0].size();
    ...
    void flowAtlantic(int i, int j, const vector<vector<int>>& matrix, vector<vector<bool>>& atlantic) {
        int n = matrix.size();
        int m = matrix[0].size();
